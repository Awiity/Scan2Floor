export default function Sidebar({
  ...
  showCloud,           setShowCloud,
  showFloorPlan,       setShowFloorPlan,
  showFloorPlanViewer, setShowFloorPlanViewer,
  modelInfo,
  backendStatus,
  cloudPoints,
  onCameraPreset,
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-section">
        ...
        <div className="project-card">
          ...
          <div className="project-meta" style={{ marginBottom: 4 }}>Matterport · XYZ + OBJ</div>
      ...
      <div className="sidebar-section">
        ...
        <div className="layer-item" onClick={() => cloudReady && setShowCloud(v => !v)}
             style={{ opacity: cloudReady ? 1 : 0.5, cursor: cloudReady ? 'pointer' : 'not-allowed' }}>
          <div className="layer-icon cloud">✦</div>
          <div style={{ flex: 1 }}>
            <div className="layer-label">Point Cloud</div>
            <div className="layer-sub">
              {cloudReady
                ? cloudPoints ? `${fmt(cloudPoints)} pts loaded` : `~1.1 M pts ready`
                : 'Preprocessing…'}
            </div>
          </div>
      ...
      <div className="sidebar-section">
        ...
        <div className="stat-row">
          ...
          <span className="stat-value">{fmt(modelInfo?.n_total ?? 114_036_775)}</span>
        </div>
        <div className="stat-row">
        ...
        <div className="stat-row">
          ...
          <span className="stat-value">1 : {modelInfo?.sample_rate ?? 100}</span>
        </div>
        <div className="stat-row">
      ...
      <div className="sidebar-section">
        ...
        <div className="cam-grid">
          {[
            ...
            <button key={key} className="cam-btn" onClick={() => onCameraPreset(key)}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="sidebar-section">
        ...
        {[
          ...
          { icon: '🚪', label: 'Door / Window Detection', note: 'M4 Next',  done: false },
          { icon: '📄', label: 'DXF Export',              note: 'M5 Next',  done: false },
          ...
          <div key={label} style={{
            ...
            <span>{icon}</span>
            <span>{label}</span>
            <span style={{ marginLeft: 'auto', fontSize: 10, color: done ? 'var(--cyan)' : 'var(--purple)' }}>{note}</span>
          </div>
        ))}
      </div>
    </aside>
